...
%******referenced from lecutre notes******
idxc = find(I>0); %index for charging process
Ic = I(idxc);
Tc = T(idxc);
idxd = find(I<0); %index for discharging process
Id = I(idxd);
Td = T(idxd);
Cbatt_c = Coulombcount(Ic,Tc); %Capacity of battery computed in charging process
Cbatt_d = Coulombcount(Id,Td); %Capacity of battery computed in discharging process
Cbatt = max(Cbatt_c,Cbatt_d);
...
St = sum((V-mean(V)).^2); % totoal sum of squares around the mean, V is the measured voltage
...
for i = 2:length(s)
    id = find(SOC_d<s(i) & SOC_d>s(i-1)); % index for scaling, in discharging period
    ic = find(SOC_c<s(i) & SOC_c>s(i-1)); % index for scaling, in charging period
...
% compute SOC by using Coulomb Counting equation 
function S_O_C=SOCcc(I,T,Cbatt,SOC0)
    S_O_C=zeros(length(I),1);
    S_O_C(1)=SOC0; % initial SOC
    for i=2:length(I)
        S_O_C(i)=S_O_C(i-1)+(T(i)-T(i-1))*I(i)/Cbatt;
    end
end